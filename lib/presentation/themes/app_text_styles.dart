...
/// No custom fontFamily is set — uses platform system default
/// (Roboto on Android, SF on iOS) consistently via this single source.
class AppTextStyles {
  AppTextStyles._();
  ...
  // ===== TECHNICAL / MONOSPACE =====
  // Use for GPS coordinates, device IDs, MAC addresses — anywhere fixed-width
  // digits matter. Was previously duplicated ad-hoc in 5 different files.
  ...
  static TextStyle heading1WithColor(BuildContext context, {Color? color}) =>
      heading1.copyWith(color: color ?? _primary(context));
  static TextStyle bodyWithColor(BuildContext context, {Color? color}) =>
      bodyMedium.copyWith(color: color ?? _secondary(context));
